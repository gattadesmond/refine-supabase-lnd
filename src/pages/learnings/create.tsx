...
export const LearningMaterialsCreate = () => {
  const {
    formProps,
    saveButtonProps,
    form,
  } = useForm<Learning, HttpError, Learning>({
    redirect: "list", // Redirect to list after create
  });

  const { mutate: addAuthors } = useCreateMany();

  // Override formProps để thêm authors handling trước khi submit
  const enhancedFormProps = {
    ...
    onFinish: (values: Learning) => {
      ...
      // Store authors in form data for use in onSuccess callback
      (form as any).authorsToCreate = authors;
      
      return formProps.onFinish?.(dataWithoutAuthors);
    },
    onSuccess: (data: any) => {
      // Handle authors creation after main record is created
      const authorsToCreate = (form as any).authorsToCreate;
      if (authorsToCreate && authorsToCreate.length > 0) {
        addAuthors({
          resource: "learnings_authors",
          values: authorsToCreate.map((author: any) => ({
            learning_id: data.data.id,
            author_id: author,
          })),
        });
      }
